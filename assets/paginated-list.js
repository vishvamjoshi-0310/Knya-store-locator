...
export default class PaginatedList extends Component {
  ...
  connectedCallback() {
    ...
    // Listen for filter-changed event (dispatched by renderProductGridContainer)
    // This handles the case when quick-filter updates the grid innerHTML directly
    document.addEventListener('filter-changed', this.#handleFilterChanged);
  }
  ...
  disconnectedCallback() {
    ...
    // Remove the filter-changed listener
  ...
  #observeViewMore() {
    ...
    if (!this.infinityScrollObserver) {
      ...
      // Disconnect all current observations before re-observing
      // This ensures we're observing the current DOM elements after filter updates
      this.infinityScrollObserver.disconnect();
    }
  ...
  /**
   * Handle filter-changed event (dispatched by renderProductGridContainer)
   * This ensures the observer is re-initialized after quick-filter updates the DOM
   */
  #handleFilterChanged = () => {
    // Wait for DOM to be updated and refs to be refreshed
    requestAnimationFrame(() => {
      // Use a small delay to ensure refs are updated after innerHTML change
      setTimeout(() => {
        if (this.isConnected) {
          this.#observeViewMore();
          // Also fetch the next page if available
          this.#fetchPage('next');
        }
      }, 100);
    });
  ...
  #handleFilterUpdate = () => {
    ...
    const hadViewMoreNext = !!this.refs.viewMoreNext;
    ...
    const observer = new MutationObserver(() => {
      // Check if data-last-page changed or viewMoreNext element was added/removed
      const newLastPage = this.refs.grid?.dataset.lastPage;
      const hasViewMoreNext = !!this.refs.viewMoreNext;

      if (newLastPage !== currentLastPage || hasViewMoreNext !== hadViewMoreNext) {
        observer.disconnect();
    ...
    const { grid } = this.refs;
    if (grid) {
      // Observe the grid for data-last-page changes
      ...
      // Also observe the component itself to catch when viewMoreNext (sibling of grid) is added/removed
      ...
      // Also re-initialize the observer to ensure it works even if mutation doesn't fire
      setTimeout(() => {
        if (observer) {
          observer.disconnect();
        }
        // Ensure component is still connected before re-initializing
        if (this.isConnected) {
      ...
      // If grid doesn't exist yet, wait a bit and try again
      requestAnimationFrame(() => {
        if (this.isConnected && this.refs.grid) {
          this.#observeViewMore();
          this.#fetchPage('next');
        }
      });
